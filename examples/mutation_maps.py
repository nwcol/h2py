...
mut_map_file = "real_data/roulette_mutation_map_chr22.csv.gz"
bed_file = "real_data/roulette_map_coverage_chr22.bed.gz"
...
def run_msprime(graph, mut_map, out_file):
    demog = msprime.Demography.from_demes(graph)
    ts = msprime.sim_ancestry(
        samples,
        demography=demog,
        sequence_length=mut_map.position[-1],
        recombination_rate=r,
    )
    ts = msprime.sim_mutations(ts, rate=mut_map)
...
if __name__ == "__main__":
    ...
    tbl = pandas.read_table(mut_map_file, engine="python", sep=None)
    mut_rate = tbl["mutRate"]
    mut_rate[np.isnan(mut_rate)] = 0
    mut_pos = np.append(tbl["chromStart"], np.array(tbl["chromEnd"])[-1])
    mut_map = msprime.RateMap(position=mut_pos, rate=mut_rate)
    ...
    for out_file in out_files:
        run_msprime(graph, mut_map, out_file)
    sums = {}
